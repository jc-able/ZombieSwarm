...
func _bake_wall_faces(space: PhysicsDirectSpaceState3D):
	...
	for gz in range(_grid_size):
		for gx in range(_grid_size):
			...
			# Skip cells that are themselves walls/buildings — only scan from open cells
			...
			for d in dirs:
				...
				# Only scan toward neighbors that are WALL or BUILDING
				if _obstacle_type[nidx] < OBS_WALL:
					continue
				...
				if hit_count >= WALL_MIN_HITS:
					...
					# Use neighbor's obstacle_top as wall top
					var n_top: float = _obstacle_top[nidx]
...
func spawn_swarm(center: Vector3, direction: Vector3, count: int, spread: float = 1.5):
	...
	for i in range(count):
		...
		_multimesh.set_instance_custom_data(i, Color(randf_range(0.0, 2.0), 0, 0, 0))
...
func _process(delta: float):
	...
	for i in range(_instance_count):
		...
		var xform := Transform3D().rotated(Vector3.UP, f)
...
func _simulate(idx: int, delta: float):
	...
	# ── Get obstacle data for the zombie's current layer ─────────────
	# _obs_type_at / _obs_top_at are the obstacle data for this zombie's layer
	var _obs_type_at := _upper_obs_type[gidx] if on_upper else _obstacle_type[gidx]
	var _obs_top_at := _upper_obs_top[gidx] if on_upper else _obstacle_top[gidx]
	...
	# ── Direction logic ──────────────────────────────────────────────────
	if cur_blocked:
		# Use wall normal for smoother sliding if available at zombie's current cell
		var wall_f := int(_wall_faces[gidx]) if gidx < _wall_faces.size() else 0
		if wall_f > 0 and not on_upper:
		...
		if wall_f == 0 or on_upper:
			# Standard steering: flow field + perpendicular checks
			var flow_dir := Vector3(_flow_x[gidx], 0, _flow_z[gidx])
			if on_upper:
				# Upper-level zombies use base direction (flow field is ground-level)
				flow_dir = _base_direction
			var perp := Vector3(flow_dir.z, 0, -flow_dir.x)

			# Check perpendicular obstacles using the correct layer
	...
	# Re-determine layer after movement
	...
	# ── Ceiling clamp (prevent clipping through upper layer from below) ──
	if not on_upper:
...
func _setup_placeholder_mesh() -> ShaderMaterial:
	...
	var mat := ShaderMaterial.new()
	mat.shader = shader
	mat.set_shader_parameter("body_color", Color(0.35, 0.55, 0.25, 1.0))
	return mat
